...
async function idbGet(store: string, key: string): Promise<unknown | null> {
    const db = await openDb();
...
export async function saveImage(args: {
    ...
    if (existingId) {
        ...
        const metaRecord = await idbGet(IMAGES_STORE, `${existingId}:meta`);
        const sizeBytes = (metaRecord && typeof metaRecord === 'object' && 'sizeBytes' in metaRecord) 
            ? (metaRecord as { sizeBytes: number }).sizeBytes 
            : imageBlob.size;
...
export async function getImageBlob(imageId: string): Promise<Blob | null> {
    ...
    const result = await idbGet(IMAGES_STORE, imageId);
    return (result instanceof Blob) ? result : null;
}

export async function getThumbBlob(imageId: string): Promise<Blob | null> {
    const result = await idbGet(THUMBS_STORE, imageId);
    return (result instanceof Blob) ? result : null;
}
